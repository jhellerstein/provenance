...
The operator $\wedge$ denotes predicate-level \emph{sequential refinement} and is
therefore generally non-commutative.
When reasoning about \emph{depth}, however, we will often group predicates into
\emph{layers} whose predicates commute (i.e., can be applied in any order without
changing the result).
...
\begin{definition}[Sequencing of layers (notation)]
  \label{def:layer-seq}
  Let $L_1,\ldots,L_k$ be multisets of interpretive predicates.
  We write $L_1 \seq L_2 \seq \cdots \seq L_k$
  for the interpretation obtained by applying the predicates of $L_1$ (in any
  ...
  \]
  where $\Spec \mid L$ abbreviates refinement by all predicates in $L$ in any
  order.
...
\subsection{Algebraic Structure of Interpretations}
\label{sec:algebra}

Interpretations refine a specification through a sequence of semantic
commitments.
This process exhibits a layered algebraic structure, which we summarize here to
clarify the relationship between interpretation provenance and classical algebraic
provenance.

\paragraph{Sequential conditioning across layers.}
Between layers, interpretive predicates compose by sequential conditioning.
This composition is generally dependent and non-commutative: applying one
predicate may enable or disable another.
...
Within a single layer, interpretive predicates commute.
Order of application does not affect the resulting restriction of admissible
outcomes.
As a result, predicates within a layer form a \emph{conditional commutative
  monoid}, where conjunction corresponds to joint semantic commitment and the
identity element is the trivial predicate.

The carrier of this monoid is conditional on prior layers: which predicates are
available, and which outcomes they act upon, depends on the interpretation prefix.

\paragraph{Special case: semiring structure.}
In special cases—most notably when interpretive predicates are pointwise and
...
Once a resolving interpretation $I$ is fixed, all semantic ambiguity has been
eliminated.
The conditioned specification $\Spec \mid I$ is single-valued and monotone: no
further interpretive predicates can rule out admissible outcomes.
...
Accordingly, interpretation provenance pairs a resolving interpretation with
...
\[
  ...
  \bigr)
\]
All predicates in $I_{\mathsf{in}}$ commute; thus $I_{\mathsf{in}}$ forms a single
interpretation layer (depth~1).
...
\begin{figure*}[t]
  ...
  \begin{tabular}{@{}p{0.23\linewidth} p{0.54\linewidth} p{0.12\linewidth}@{}}
    ...
    \midrule
    \textbf{Case}                & \textbf{One resolving interpretation} & \textbf{Depth} \\
    \midrule
    SER robust $b$               &
  ...
  \caption{
    Interpretation summaries for examples.
    ...
    resolving interpretation, while stable semantics admits multiple incompatible
    ones.
    (\textbf{B}) For serializable transactions, many serial interpretations may
    justify the same outcome; we therefore display one resolving interpretation
...
corresponding to the layered interpretation
\[
  \varphi_{\seal}(S_1) \seq \varphi_{\seal}(S_2) \seq \cdots \seq \varphi_{\seal}(S_k)
\]
% [added \seq]
...
Thus stable semantics has interpretation depth~$k{+}1$, corresponding to
\[
...
\emph{interpretation structures}.
...
Order, therefore, is not a primitive feature of the execution.
It is a \emph{semantic commitment} used to justify an outcome.
...
Classical data provenance assumes a fixed execution outcome.
Transactional systems invalidate this assumption.
A single history may admit multiple outcomes, and tuple-level provenance questions
must therefore range over alternative semantic resolutions of concurrency.
...
As in Sandbox~I, this single example suffices to exhibit multiple interpretation
shapes under different semantics.

\subsection{Serializability as Interpretation Shape}
...
The primitive interpretive predicates in this setting are \emph{ordering
  predicates}.
...
order; a fully resolving interpretation induces a total order.

Crucially, ordering predicates commute.
Committing to $T_1 \prec T_2$ and to $T_2 \prec T_3$ can be done in either order and
has the same effect.
As a result, serializability has interpretation depth~1: all semantic commitments
...
Prior to fixing an interpretation, there is no single classical provenance question
for $d$.
Interpretation provenance records the semantic commitments under which each
explanation applies.

Figure~\ref{fig:interp-tables}(\textbf{B}, SER) summarizes representative resolving
interpretations.

\subsection{Snapshot Isolation as Interpretation Shape}
...
\paragraph{Interpretive predicates.}
An SI interpretation consists of:
...
Unlike serializability, these predicates do not commute in general: snapshot
choices for one transaction constrain admissible choices for others.
Accordingly, snapshot isolation has worst-case interpretation depth $O(|H|)$,
reflecting a sequence of non-commuting snapshot commitments, which can be written
schematically as
\[
...
Figure~\ref{fig:interp-tables}(\textbf{B}, SI) records one such resolving
interpretation for the running example.
Once an SI interpretation is fixed, the specification again becomes single-valued,
...
Interpretation provenance makes these differences explicit, while classical
...
execution traces, including discretization, causal graph construction, and
sensitivity to unobserved variables
\cite{markakis2024sawmill,chen2025causalens,markakis2024press,zeng2024validity,lai2025seercuts}.
These systems typically take logs as causal histories and focus on summarizing,
...
Interpretation provenance does not perform causal inference; rather, it provides
...
may be robust across a class of causal interpretations, or may depend critically
...
This appendix clarifies the algebraic structure induced by interpretations and
explains why classical algebraic provenance applies once semantic ambiguity has
been resolved.
The goal is not to develop a new provenance algebra, but to situate interpretation
...
Interpretations refine a specification by applying interpretive predicates via
\emph{sequential conditioning}.
Given a specification $\Spec$ and predicates $\varphi_1,\varphi_2$, refinement
\[
...
Interpretive predicates therefore do \emph{not} combine explanations, but restrict
the space in which explanations are defined.

\subsection{Commutative Structure Within a Layer}

Within a single interpretation layer, predicates commute by construction.
Fix a specification prefix $\Spec^{\ell}$ obtained after resolving all prior layers.
Let $L$ be a set of interpretive predicates applicable at that layer.
...
\]
Thus predicates in $L$ form a \emph{commutative monoid} under conjunction, with the
identity given by the trivial predicate that performs no restriction.

This monoid is \emph{conditional}: the available predicates and their effect depend
on the interpretation prefix.
No global algebra over predicates is assumed or required.

\subsection{Special Case: Semiring Structure}

In special cases, the commutative monoid within a layer extends to a semiring.
This occurs when:
(i) predicates are pointwise over admissible outcomes; and
...
The key transition occurs once a resolving interpretation $I$ has been fixed.
...
Interpretive predicates no longer apply, and semantic conditioning ceases.
...
instance.
Once interpretation is complete, this is exactly the situation we obtain.

\begin{proposition}[Resolved specifications admit semiring provenance]
  \label{prop:semiring}
  If $\Spec \mid I$ is resolved and monotone, then derivational provenance over
  $\Spec \mid I$ is captured by a commutative semiring.
\end{proposition}

\paragraph{Justification.}
Because $\Spec \mid I$ is single-valued, there is no further semantic elimination of
outcomes.
Because it is monotone, derivations combine locally:
alternative derivations combine by addition, and joint use of facts combines by
multiplication.
This is precisely the algebraic regime studied in classical provenance work.

Importantly, $\Spec \mid I$ need not syntactically be a Datalog program.
However, it is algebraically equivalent, for provenance purposes, to monotone query
evaluation over a fixed instance.
This equivalence suffices to justify the use of semiring provenance and aligns our
framework with existing algebraic results.
...
interpretation provenance recovers the algebraic conditions required for semiring
provenance by explicitly discharging semantic ambiguity.

Seen this way, interpretation theory serves as a semantic prelude to algebraic
provenance.
...
This appendix instantiates interpretation provenance for the transactional sandbox
used in Section~\ref{sec:transactions}.
As in the Datalog appendix, we make explicit the interpretive predicates,
their dependency structure, and the resulting provenance annotations.
The goal is to justify the interpretation shapes and depths summarized in
Figure~\ref{fig:interp-tables}(\textbf{B}).

Throughout, interpretive predicates rule out admissible executions and therefore
...
The primitive interpretive predicates are \emph{ordering predicates}
...
Hence serializability has interpretation depth~1.
...
\paragraph{Resolving interpretations.}
Two valid resolving interpretations under serializability are:
\[
  I_{\mathsf{in}} \triangleq
...
\paragraph{Provenance.}
Once a resolving interpretation is fixed, provenance is purely derivational.
...
Interpretation provenance records the semantic assumption under which the
...
Thus interpretation provenance distinguishes semantic explanations even when the
...
Snapshot isolation (SI) resolves concurrency using a different basis of semantic
commitments.
Rather than committing to a total order, an SI interpretation assigns each
...
Interpretively, this yields two kinds of predicates:
...
interpretation structure whose depth grows with the history.
...
\paragraph{Resolving interpretation.}
One admissible SI interpretation is:
\[
  \begin{aligned}
...
Interpretation provenance makes this explicit.
Restricting attention to SI interpretations whose snapshot predicates admit a
...
transaction.
Thus SI has worst-case interpretation depth $O(|H|)$, as recorded in
Figure~\ref{fig:interp-tables}(\textbf{B}).
...
an interpretation imposes on admissible outcomes.
...
but interpretation provenance still cleanly separates observable evidence from
...
ambiguous prior to interpretation.
...
\paragraph{Summary.}
This appendix demonstrates that interpretation provenance applies naturally to
systems diagnostics.
