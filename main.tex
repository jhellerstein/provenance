...
invariance, or sensitivity to cross-spec semantic change—are treated explicitly and
...
class?”), cross-spec semantic-change queries (Section~\ref{sec:robustness}), and
...
Appendix~\ref{sec:appendix-datalog} shows how cross-spec semantic-change queries
...
cross-spec semantic change (Section~\ref{sec:robustness}).
...
Semantic change is a cross-spec question; see Section~\ref{sec:robustness}.
...
Semantic change in transactional settings is a cross-spec question; see
...
Throughout the paper we consider two distinct kinds of semantic questions.
\emph{Intra-spec} questions range over resolving interpretations of a fixed specification,
asking which conclusions are stable, contingent, or explainable under different resolutions of ambiguity.
\emph{Cross-spec} questions compare conclusions across different specifications,
asking whether results persist under semantic change.
Intra-spec questions ask “what holds under different resolutions of the same semantics,”
whereas cross-spec questions ask “what survives when the semantics themselves change.”
Although both are analyzed using interpretation provenance, they quantify over different spaces and should
not be conflated.

We write robustness queries as quantification over resolving interpretations $I$ of a fixed $\Spec$, and semantic-change queries as comparisons across distinct specifications $\Spec$ and $\Spec'$.
...
In the interpretation framework (Section~\ref{sec:robustness}), semantic change is explicitly cross-spec: it
compares conclusions obtained under different semantic regimes.
We model such regimes via conditioning on (potentially different) resolving
interpretations.
Let $\Spec$ be an ambiguous specification, and let $I$ and $I'$ be two resolving
interpretations representing different semantic choices.
...
Cross-spec semantic change compares two specific resolving interpretations.
...
This example is a concrete instance of cross-spec semantic change in the sense of
