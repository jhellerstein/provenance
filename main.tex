...
sequential execution in that order yields the same final state and is
consistent with the history’s ordering constraints.
...
Once sufficient ordering commitments have been made, query evaluation is
monotone and classical semiring provenance applies.
Interpretation provenance captures the semantic structure that exists
\emph{before} such commitments are fixed.

We reuse the running example from Section~\ref{sec:intro} and
Appendix~\ref{sec:appendix-concurrency}, focusing on the contingent tuple $d$
inserted by $T_4$ and deleted by $T_5$.

Under the resolving interpretation
\[
...
question for $d$.
Interpretation provenance makes this dependence explicit by recording the
semantic commitments under which derivations hold:
\[
