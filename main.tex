...
invariance, or sensitivity to inter-specification semantic change—are treated explicitly and
...
\subsection{Semantic change (forward reference)}
\label{sec:txn-semantic-change}

Semantic change in transactional settings is an inter-specification question; see
Section~\ref{sec:robustness} and Appendix~\ref{sec:appendix-concurrency}.
...
We return to mixed-regime systems (e.g., SER + SI) in Section~\ref{sec:robustness}, where we treat inter-specification interaction as an explicit policy layer.
...
\paragraph{Two classes of questions.}
We distinguish \emph{intra-specification} questions, which vary resolving interpretations $I$ for a fixed $\Spec$,
from \emph{inter-specification} questions, which compare results across distinct specifications $\Spec$ and $\Spec'$.
Both are analyzed using interpretation provenance, but they quantify over different dimensions: interpretation choice versus specification change.
Intuitively, intra-specification questions ask what holds under different resolutions of the same semantics, whereas inter-specification questions ask what survives when the semantics themselves change.
We use the terms intra-specification and inter-specification throughout this section.

\paragraph{Provenance checks.}
Interpretation provenance supports concise schematic constraints for both
question families.

\emph{Intra-specification (robustness).}
Fix a specification $\Spec$ and a class $\mathcal{I}$ of resolving interpretations
of $\Spec$.
A tuple $t$ is \emph{robust} with respect to $\mathcal{I}$ iff
\[
  \forall I \in \mathcal{I}.\; t \in (\Spec \mid I)(H).
\]
That is, $t$ is justified under every admissible resolution of the same
semantic specification.

\emph{Inter-specification (semantic change).}
Given specifications $\Spec$ and $\Spec'$, a tuple $t$ is \emph{preserved} across
semantic change iff
\[
  \forall I \in \mathcal{I},\; \forall I' \in \mathcal{I}'.\;
  \bigl(t \in (\Spec \mid I)(H)\bigr) \;\Rightarrow\;
  \bigl(t \in (\Spec' \mid I')(H)\bigr),
\]
where $\mathcal{I}$ and $\mathcal{I}'$ range over the resolving interpretations
appropriate to $\Spec$ and $\Spec'$, respectively.
This check compares conclusions across specifications, rather than across
resolutions of a single specification.

Appendices~\ref{sec:appendix-datalog} and~\ref{sec:appendix-concurrency} instantiate
both forms of provenance checks on our running examples.

\smallskip
Interpretation provenance also reframes \emph{why-not} provenance.
Classical why-not explanations account for absence relative to a fixed semantics.
Under interpretation provenance, absence may instead be explained by the absence
of any resolving interpretation that admits the result, or by the fact that only
interpretations excluded by policy or specification choice would justify it.

We map the provenance question space enabled by interpretation provenance; we do not propose algorithms or completeness results.
In particular, we focus on intra-specification robustness and inter-specification semantic change.
...
In the interpretation framework, semantic change is an inter-specification question:
it compares conclusions obtained under different specifications $\Spec$ and $\Spec'$.
We model the semantic regimes under which each specification is interpreted via conditioning.

\subsubsection{Mixed-spec execution as policy}
Mixed regimes (e.g., some transactions under SI and others under SER) are not a
semantic primitive; they require an explicit policy for how results justified under
one specification are treated under another.
Meaningful options include: (i) \emph{Reject}, (ii) \emph{Mask-as-missing}, (iii) \emph{Annotate}
(carry interpretation provenance/certificate metadata), or (iv) \emph{Accept}.
We do not advocate a particular policy.
Interpretation provenance supports all four by making explicit the interpretive predicates/commitments
that justify each result, making the policy choice explicit and checkable.

The corresponding preservation check is given schematically in the provenance-checks paragraph above.
...
Robustness is an intra-specification question: whether a conclusion holds across an entire
family of admissible interpretations of a fixed $\Spec$.
...
The corresponding quantification over $I \in \mathcal{I}$ is given schematically in the provenance-checks paragraph above.
