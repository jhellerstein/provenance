...
Interpretations resolve semantic ambiguity by imposing semantic commitments that
are not independent: some commitments can only be made once others are fixed.
This structure is captured by the dependency graph over interpretive predicates
introduced earlier.

Let $G = (\Phi, \rightarrow)$ be the dependency graph for a specification $\Spec$,
where $\varphi \rightarrow \psi$ indicates that committing to $\psi$ requires
$\varphi$ to have already been resolved.
A \emph{layer} is an antichain in $G$ whose elements depend only on predicates in
earlier layers.
Intuitively, a layer $L_i$ groups semantic commitments that are mutually independent,
given the commitments made in lower layers $L_1, \ldots, L_{i-1}$.

Interpretation can therefore be understood as refinement along successive layers
of this dependency structure.
However, neither subsequent semantic refinement nor provenance queries require
access to the full refined specification.
Instead, it suffices to retain summaries that preserve only the semantic
distinctions exposed at each layer boundary.
We call these summaries \emph{layer certificates}.
...
\begin{definition}[Certificate]
  ...
  A certificate for $\varphi$ is a predicate $C : O \to \{\mathsf{true},\mathsf{false}\}$
  such that for all histories $H$ and all nonempty $S \subseteq O$,
  \[
    \varphi(H,S) \subseteq \{\, o \in S \mid C(o) \,\}.
  \]
  We write $\varphi \models C$.
\end{definition}

Certificates are lossy summaries that may admit false positives: they exclude only
those outcomes that interpretation must exclude, without encoding the full
resolution process.
...
Examples include interpretation-conditioned classical data provenance queries (why, why-not, etc.),
and the robustness and preservation queries we introduce in Section~\ref{sec:robustness}.
...
\begin{definition}[Layer Certificate]
  ...
  sufficient for $(J,\mathcal{Q})$, where $J$ is the remaining interpretation
  after layer $L_i$ and $\mathcal{Q}$ is the intended family of provenance queries.
\end{definition}
...
observability.
Tracing systems such as Dapper typically retain worst-case information---full histories---in order to
...
Figure~\ref{fig:interp-tables}(A) summarizes the interpretations for these regimes,
making explicit which interpretive predicates are sufficient to resolve the
specification under each semantics.
When a semantics admits multiple incompatible resolutions, we enumerate them;
when it admits a unique resolution, we record that interpretation alone.
Appendix~\ref{sec:appendix-datalog} contains full details, including the
derivation of conditioned provenance values. We proceed with an
...
\begin{figure*}[t]
  ...
  \begin{tabular}{@{}p{0.20\linewidth} p{0.72\linewidth}@{}}
    ...
    \midrule
    Stratified    &
    $\varphi_{\mathrm{stab}(p)}$                                                \\
    %
    Well-founded  &
    $\varphi_{\mathrm{stab}(p)} \wedge \varphi_{p(c)=\mathbf{t}} \wedge \varphi_{q(c)=\mathbf{f}}
    \wedge \varphi_{r(c)=\mathbf{u}} \wedge \varphi_{s(c)=\mathbf{u}}$          \\
    %
    Stable        &
    $(\varphi_{\mathrm{stab}(p)} \wedge I_{\mathrm{stab}}^{(r)}) \;\text{or}\;
    (\varphi_{\mathrm{stab}(p)} \wedge I_{\mathrm{stab}}^{(s)})$ (incompatible) \\
    \bottomrule
  \end{tabular}
  ...
  \caption{
    ...
    incompatible resolving interpretations.
    ...
    as a semantic witness, since the existence (rather than enumeration) of such
    interpretations is what matters.
    % add to caption (one sentence):
    The SI row abbreviates snapshot choices ($\snap(\cdot)$) and the first-committer-wins
    rule ($\FCW$).
...
\paragraph{Interpretive predicates for negation.}
...
false, or (when the outcome domain is lifted) undefined. Only commitments that
rule out admissible outcomes count as interpretive predicates;
consequences that hold in all remaining outcomes are treated derivationally.
...
commitments.
As a result, cycles of interpretive predicates with negation are ambiguity-sensitive and may
fail to commute.
...
In our running example, we temporarily set aside predicates $r(c)$ and $s(c)$,
which form a negation cycle, and focus on the stratified fragment defining $p(c)$
and $q(c)$.
Predicate $p(c)$ lies in a lower stratum derived from the EDB,
while $q(c)$ lies in a higher stratum, depending negatively on $p(c)$.

The central semantic issue is not the computation of a fixpoint for $p(c)$ per se, but the
commitment that the lower stratum has \emph{stabilized}—no further
lower-stratum consequences will appear that could change the fixpoint.
...
negation is safe to apply; resolving that ambiguity requires committing to a
closed world for the lower stratum.

Accordingly, resolution proceeds by a single interpretive predicate,
$\varphi_{\mathrm{stab}(p)}$, committing that the lower stratum defining $p(c)$ has
stabilized. Once this commitment is made, negation is well-defined and subsequent
derivations proceed without further semantic resolution,as summarized in the first row of
...
For our example, the stratified prefix remains unchanged: resolution commits only
to stabilization of the lower stratum via $\varphi_{\mathrm{stab}(p)}$.
Negation over $p(c)$ is then forced, while the remaining cycle between $r(c)$ and
$s(c)$ is handled by lifting the outcome domain.

The resulting interpretation produces a unique element of $2^O$: there is a single
admissible outcome.
...
In our example, resolution again begins by committing to stabilization of the lower stratum via
$\varphi_{\mathrm{stab}(p)}$.
Beyond this commitment, the cycle between $r(c)$ and $s(c)$ now admits multiple incompatible
resolutions.
...
predicates commute), snapshot isolation has interpretation depth
$O(|H|)$ in the worst case, since each transaction may require an independent
snapshot commitment.
...
admissible under $\Spec$ and $\Spec'$ respectively.
...
Interpretation provenance provides a concrete handle on this question.
An SI interpretation explicitly records snapshot choices and commit-admissibility
constraints.
From these predicates, one can check whether the implied visibility and conflict
relations admit a topological sort that constitutes a serial execution.
If so, the outcome is preserved under strengthening from SI to serializability;
if not, the outcome is inherently non-serializable. Appendix~\ref{sec:appendix-concurrency}
works through a detailed example.
...
that the lower stratum defining $p$ has stabilized for the current history.
In the stratified fragment, this commitment suffices to eliminate all semantic
ambiguity: once the lower stratum is fixed, the truth of $\neg p(c)$ is determined
derivationally.

Accordingly, a resolving interpretation for the stratified fragment is simply
\[
  I_{\mathrm{strat}} \;\triangleq\; \varphi_{\mathrm{stab}(p)} .
\]

There is no further interpretive dependency: negation in the higher stratum does
not rule out admissible outcomes beyond those already excluded by stabilization.

Over the resolved specification $\Spec \mid I_{\mathrm{strat}}$, $p(c)$ is derived
from $a(c)$ with ordinary derivational provenance
...
Under the given history $H$ with $a(c)$, $q(c)$ does not hold under stratified
semantics; this absence is derivational and requires no additional semantic
commitment.

\subsubsection{Well-founded semantics: explicit interpretation provenance}

Well-founded semantics applies to programs with negation cycles by resolving
ambiguity through a lifted outcome domain.
Rather than choosing among multiple two-valued outcomes, it commits certain atoms
to the value $\mathbf{u}$ (undefined).

\paragraph{Interpretation provenance (example).}
Let $\mathbf{t},\mathbf{f},\mathbf{u}$ denote the well-founded truth values
true/false/undefined.
A resolving interpretation for $(P,H)$ under WFS can be written as
\[
...
These interpretive predicates are outcome-level commitments that collapse the
space of admissible models to a single partial interpretation.
There is no remaining semantic ambiguity among outcomes, and hence no
nontrivial dependency structure among these predicates.
...
The key distinction from the stratified case is that resolution is achieved not by
staging commitments, but by lifting the outcome domain to explicitly represent
residual ambiguity.

\subsubsection{Stable semantics: explicit interpretation provenance}

Stable model semantics resolves negation cycles by selecting total two-valued
models.
In the presence of a negation cycle, this yields multiple incompatible resolving
interpretations.

\paragraph{Interpretation provenance (example).}
For our example with $a(c)$, the lower stratum again forces $p(c)$ to be true.
The negation cycle between $r(c)$ and $s(c)$ admits two stable resolutions:
\[
  I_{\mathrm{stab}}^{(r)} \;\triangleq\;
  \varphi_{p(c)=\mathbf{t}} \wedge
  \varphi_{q(c)=\mathbf{f}} \wedge
  \varphi_{r(c)=\mathbf{t}} \wedge
  \varphi_{s(c)=\mathbf{f}},
\]
\[
  I_{\mathrm{stab}}^{(s)} \;\triangleq\;
  \varphi_{p(c)=\mathbf{t}} \wedge
  \varphi_{q(c)=\mathbf{f}} \wedge
  \varphi_{s(c)=\mathbf{t}} \wedge
  \varphi_{r(c)=\mathbf{f}} .
\]

These interpretations are mutually incompatible conditionings of the same
program and history.
Unlike well-founded semantics, stable semantics does not prescribe a unique
resolution of the cycle.

Within either resolved outcome, derivational provenance is ordinary:
\[
  \mathsf{IP}(p(c)) \;\triangleq\; (I_{\mathrm{stab}}^{(r)},\, \Prov(a(c)))
  \quad\text{or}\quad
  (I_{\mathrm{stab}}^{(s)},\, \Prov(a(c))) .
\]
...
Thus, in the stable setting, interpretation provenance must explicitly record
which resolved interpretation was assumed before derivational provenance applies.

\subsubsection{Semantic change example: negation semantics}

This example illustrates inter-specification semantic change.
Holding the program and extensional history fixed, different negation semantics
induce different spaces of resolving interpretations and hence different
...
Under WFS, the unique resolving interpretation is
\[
  I_{\mathrm{wfs}}
  \;\triangleq\;
  \varphi_{p(c)=\mathbf{t}} \wedge
  \varphi_{q(c)=\mathbf{u}} \wedge
  \varphi_{r(c)=\mathbf{u}} .
\]
Here $p(c)$ is true, while $q(c)$ and $r(c)$ are undefined.

\paragraph{Stable model semantics.}
Under stable semantics, the same program admits two resolving interpretations:
\[
  I^{(q)}_{\mathrm{stab}}
  \;\triangleq\;
  \varphi_{p(c)=\mathbf{t}} \wedge
  \varphi_{q(c)=\mathbf{t}} \wedge
  \varphi_{r(c)=\mathbf{f}},
  \qquad
  I^{(r)}_{\mathrm{stab}}
  \;\triangleq\;
  \varphi_{p(c)=\mathbf{t}} \wedge
  \varphi_{r(c)=\mathbf{t}} \wedge
  \varphi_{q(c)=\mathbf{f}} .
\]

The conclusion $p(c)$ is preserved across semantics, while $q(c)$ and $r(c)$ are
not.
Interpretation provenance makes this preservation (and non-preservation)
explicit.
...
derivational provenance of $p(c)$ need only preserve the commitments that make
$p(c)$ well-defined.
However, answering a semantic-change query such as
``Would $q(c)$ become true under stable-model semantics?''
requires a strictly stronger certificate: it must preserve the unresolved
negation structure of the cycle, not merely a particular resolution of it.

This illustrates the query-parametric nature of certificate sufficiency
(Section~\ref{sec:certificates}): supporting richer provenance questions can only
increase the information that must be retained.


\subsection{Full Transactional Provenance Examples}
\label{sec:appendix-concurrency}

This appendix works through the transactional sandbox examples in full detail.
We begin with serializability, which uses ordering predicates as its interpretive
basis, and then extend the same history to snapshot isolation, which requires a
richer interpretation structure.
