...
% ---------- Notation ----------
% TODO: Add theorem showing that determination resolution
% induces history-monotonicity (moved from complexity paper §C for PODS submission).
% This is the foundational result motivating why classical provenance (which assumes
% monotonicity) becomes applicable after determination resolution.
...
% \item It introduces \emph{determination provenance} as a semantic layer that
% records the commitments under which conclusions become well defined, and shows how
% classical algebraic provenance arises as a special case once a determination is
% fixed.
...
% \item It demonstrates, through two sandboxes, how determination provenance exposes
...
% classes of provenance questions—respectively, invariance across determinations of
% a fixed specification and preservation across different specifications—and shows
% how determination provenance supports both.
% \end{enumerate}
...
We model these commitments as \emph{commitment predicates}, understood as
...
% over commitments.
% \begin{definition}[Determination dependency graph]
%   A determination $I = \varphi_1 \wedge \cdots \wedge \varphi_m$ has a directed
%   acyclic \emph{determination dependency graph} $G=(\Phi, \rightarrow)$ where $\Phi = \{\varphi_1,\ldots,\varphi_m\}$
...
written $o \sim_{I,H} o'$, if no commitment in $I$ distinguishes between them at $H$.

\begin{definition}[Determination-Induced Quotient]
  For each history $H$, determination $I$ induces a quotient
  \[
    O_H^I \big/ {\sim_{I,H}},
  \]
...
Outcomes ruled out by the determination are not in $O_H^I$, and hence simply absent from the quotient.

\begin{remark}[History-indexed quotients]
  Because commitments may depend on the history and on intermediate
  admissible sets, equivalence under a determination is defined relative to a
  history.
  Determinations therefore induce a family of quotients indexed by histories,
...
\subsection{Summary}
Determination provenance pairs a resolving determination with classical
provenance computed under that determination.
Once a determination is fixed, provenance is classical by construction: it is
...
Figure~\ref{fig:interp-tables}(\textbf{A}) summarizes these determinations and
...
Figure~\ref{fig:interp-tables}(\textbf{A}, WFS) records the limit determination
...
Accordingly, snapshot isolation has worst-case determination depth $O(|H|)$,
...
When the space of determinations is finite or otherwise measurable, one can ask
quantitative questions—for example, what fraction of admissible determinations
justify a conclusion, or how often a conclusion is preserved across semantic
changes.
Determination provenance provides the semantic structure needed to formulate such
questions, though it does not require a measure over determinations.
...
Determination provenance is complementary: rather than assuming resolved
...
(i) it fails under a particular resolved determination;
(ii) it fails under all resolving determinations of the fixed specification
$\Spec$; or
(iii) it holds under some determinations of a \emph{different} specification
$\Spec'$—for example, under a different negation semantics or isolation level—but
under none of the determinations admitted by $\Spec$ itself.
...
Determination provenance makes the first two cases explicit.
...
determinations of $\Spec$, rather than in terms of blocked derivations.
...
determination provenance alone.
