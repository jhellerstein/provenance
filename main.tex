...
For the stratified fragment of our example, $p(c)$ lies in a lower stratum (derived
from EDB), and $q(c)$ lies in a higher stratum (depending on $\neg p(c)$).
...
consequences remain that could change the truth of $p(c)$.
...
$\{p(c) \leftarrow a(c).\;\,\; q(c) \leftarrow \neg p(c).\}$ eliminates the negation cycle and
...
\]
(These $\varphi$ are meta-level semantic commitments about admissible outcomes, not object-language Datalog atoms, and they operate on the outcome space induced by the program and history.)
...
\qquad
\mathsf{IP}(q(c)) \text{ undefined},
\qquad
\mathsf{IP}(r(c)) \text{ undefined}.
\]
...
was chosen:
\[
\mathsf{IP}(q(c)) \triangleq (I^{(q)}_{\mathrm{stab}},\, 1),
\qquad
\mathsf{IP}(r(c)) \triangleq (I^{(r)}_{\mathrm{stab}},\, 1).
\]



\paragraph{Robustness and contingency.}
Fact $p(c)$ is \emph{robust}: it holds in every admissible semantic world under both
well-founded and stable semantics.
By contrast, $q(c)$ and $r(c)$ are \emph{semantically contingent}: each holds in some
...
Interpretation provenance makes this dependence explicit.
The absence of $q(c)$ under well-founded semantics is not a failure of derivation,
but the consequence of a different interpretation regime.
Likewise, the presence of $q(c)$ under one stable model is incompatible with its
...
remains satisfied.
In this example, conclusions about $q(c)$ or $r(c)$ derived under a stable interpretation
have no counterpart under $I_{\mathrm{wfs}}$, while $p(c)$ remains valid.
...
without making a two-valued choice: it commits explicitly to leaving $r(c)$ and $s(c)$
undefined.
