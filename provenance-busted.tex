...
commitments were required to justify conclusions, and why interpretations
...
commitments under which conclusions become well defined.
...
conclusions under ambiguous semantics; (ii) shows how resolving interpretations recover classical
...
Resolution is defined \emph{pointwise} in the history.
A resolved specification does not assert the existence of a single global outcome,
but rather that for each history $H$, semantic ambiguity has been eliminated and a
unique outcome is determined for that history.
Different histories may therefore induce different outcomes.
Equivalently, resolution corresponds to determinism of the specification as a
function $\Hist \to O$, not to uniqueness of elements of $O$ itself.
...
Interpretations resolve semantic ambiguity by restricting and identifying
admissible outcomes.
For a history $H$ and interpretation $I$, resolution induces a quotient
\[
  O_H^I \big/ {\sim_{I,H}},
\]
which captures exactly which admissible outcomes are semantically distinguished.
...
policy, or usability—thereby collapsing distinctions among admissible outcomes.

As a result, some conclusions become invariant across all resolving
interpretations precisely because the available predicates never discriminate
between the outcomes on which those conclusions would differ.
This phenomenon motivates the robustness questions studied in
Section~\ref{sec:robustness}: robustness diagnoses conclusions that persist
\emph{because} the chosen interpretive vocabulary declines to make finer semantic
distinctions.
...
We instantiate interpretation provenance in a familiar setting: Datalog with
negation.
Our goal is not to propose or compare negation semantics, but to use classical
semantics as a controlled environment for observing how a \emph{fixed}
specification can admit multiple admissible meanings, and how those meanings are
resolved by different patterns of interpretive commitment.

Negation introduces semantic dependency: whether an atom is justified may depend
on the absence of other atoms whose status is not yet determined.
Interpretation provenance makes these dependencies explicit by recording not only
which conclusions are drawn, but the semantic commitments under which they are
valid.

We examine three standard semantics—stratified, well-founded, and stable.
Figure~\ref{fig:interp-tables}(A) summarizes, for each, the interpretive predicates
sufficient to resolve the specification.
Worked interpretations, dependency graphs, and conditioned provenance values
appear in Appendix~\ref{sec:appendix-datalog}.

\paragraph{Running example.}
Consider the Datalog program $P$ over a history $H$ containing the single
extensional fact $a(c)$:
\begin{align*}
  p(c) &\leftarrow a(c).\\
  q(c) &\leftarrow \neg p(c).\\[1mm]
  r(c) &\leftarrow \neg s(c).\\
  s(c) &\leftarrow \neg r(c).
\end{align*}
This instance contains both a stratified fragment
($q(c)$ depends negatively on $p(c)$) and a cycle through negation
($r \leftrightarrow \neg s$).
We use it throughout to contrast how different semantics resolve the same
underlying ambiguity.

\subsection{Interpretive Predicates for Negation}

Across all three semantics, resolution proceeds by applying \emph{interpretive
predicates} that constrain admissible models.
In this sandbox, such predicates commit to semantic facts about atoms—e.g.,
that an atom is true, false, or (under lifted semantics) undefined.

These commitments are not local to individual derivations.
They depend on which outcomes remain admissible and may enable or preclude further
commitments.
As a result, interpretive predicates for negation are ambiguity-sensitive and need
not commute, giving rise to dependency structure and interpretation depth.

\subsection{Stratified Semantics}

Stratified semantics applies when the dependency graph contains no cycles through
negation, allowing semantic commitments to be staged by strata.

In our example, we focus on the stratified fragment defining $p(c)$ and $q(c)$ and
temporarily ignore the cycle on $r(c)$ and $s(c)$.
The key semantic issue is not computing a fixpoint, but committing that the lower
stratum has \emph{stabilized}—that no further consequences remain that could alter
the truth of $p(c)$.
Only after this commitment is negation safe to apply.

Resolution therefore proceeds via two staged interpretive predicates:
$\varphi_{\mathrm{stab}(p)}$, committing stabilization of the lower stratum, followed
by $\varphi_{\mathrm{neg}(q)}$, committing to the stratified reading of $\neg p(c)$.
The dependency
$\varphi_{\mathrm{stab}(p)} \rightarrow \varphi_{\mathrm{neg}(q)}$
induces a single resolving interpretation, as summarized in the first row of
Figure~\ref{fig:interp-tables}(A).
Conditioned on this interpretation, derivational provenance is ordinary.

\subsection{Well-Founded Semantics}

Well-founded semantics (WFS) applies to all programs with negation, including those
with cycles.
Rather than selecting among incompatible two-valued models, WFS enlarges the
outcome domain: atoms may be \emph{true}, \emph{false}, or \emph{undefined}.

This distinction matters for interpretation provenance.
Ambiguity at the level of the specification—multiplicity in $\Spec(H) \subseteq
O$—is resolved by interpretation.
By contrast, WFS internalizes ambiguity by lifting the outcome domain $O$ itself,
producing outcomes that are intentionally partial and meaningful to the
application.

In our example, the stratified prefix remains unchanged:
$\varphi_{\mathrm{stab}(p)} \rightarrow \varphi_{\mathrm{neg}(q)}$.
The cycle between $r(c)$ and $s(c)$ induces a mutual dependence that cannot be
broken by staging.
WFS resolves this deterministically by committing both atoms to $\mathbf{u}$.

The resulting interpretation is unique as an element of $2^O$: for each history
there is a single admissible outcome.
Resolution is achieved not by choosing among alternatives, but by lifting the
outcome domain to represent partiality explicitly.
This pattern appears in the second row of
Figure~\ref{fig:interp-tables}(A).

\subsection{Stable Model Semantics}

Stable model semantics resolves negation cycles by insisting on total two-valued
models.
When cycles arise, this forces an explicit semantic choice.

In our example, the staged prefix again applies, but the cycle between $r(c)$ and
$s(c)$ now admits multiple incompatible resolutions.
Each stable model corresponds to a distinct resolving interpretation, committing
one atom to true and the other to false.
No single interpretation refines the others.

This multiplicity of resolving interpretations is summarized in the third row of
Figure~\ref{fig:interp-tables}(A).
Interpretation provenance must therefore record which resolving interpretation was
assumed before derivational provenance can be consulted.

\paragraph{Semantic change.}
Semantic change in this sandbox—e.g., comparing well-founded and stable
semantics—is an inter-specification question.
We return to this in Section~\ref{sec:robustness}; a worked analysis appears in
Appendix~\ref{sec:appendix-datalog}.

The next sandbox mirrors these resolution patterns, but with ambiguity arising
...
As before, our goal is not to propose new correctness criteria, but to expose how
semantic ambiguity arises under a fixed specification and how that ambiguity
structures provenance.

Transactional ambiguity arises from \emph{concurrency}.
A concurrent execution does not uniquely determine which transactional effects
are observed by a query.
Interpretation provenance explains how query answers depend not only on base
updates, but on the semantic commitments used to resolve that concurrency.

\subsection{Ambiguity from Concurrency}

A transactional workload consists of a set of transactions that read and write
shared state and eventually commit.
An execution history records these operations together with irreducible ordering
constraints, but does not impose a total order.

This absence of order is the source of semantic ambiguity.
...
Interpretation provenance records not only how conclusions are derived, but the
...
They ask which conclusions hold under different admissible semantic resolutions
of the \emph{same} semantics.

\emph{Inter-specification} questions compare conclusions across distinct
specifications $\Spec$ and $\Spec'$.
They ask which conclusions persist when the semantics themselves change—for
...
A conclusion $o$ is \emph{robust} with respect to $\mathcal{I}$ iff
\[
...
Semantic change is an inter-specification question: it compares conclusions
...
It explains which semantic commitments are required for conclusions to hold
...
ones.
In such settings, explaining a conclusion requires accounting for how ambiguity
...
conclusions depend on ordering and isolation commitments.
...
Once conclusions are annotated with the interpretations under which they hold, it
...
Coarser bases yield more robust conclusions but collapse distinctions; finer
...
By recording the semantic commitments under which conclusions hold, it explains
