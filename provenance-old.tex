...
% \newcommand{\Out}{\mathcal{O}} % TODO: legacy macro; prefer \Obs for observable outcomes.
...
Practical data systems can make use of this foundation, but they also operate in a
...
These examples are situated in the database context, but are more 
generally emblematic of the gap between classical data provenance and a range of
systems research on 
...
In this paper, we illustrate interpretation provenance in two domains with fundamentally different
...
lineage~\cite{sigelman2010dapper}. Here we give an initial transactional example to
...
(22) show, via Datalog with negation, how different semantics regimes induce
...
query results; and (iv) identify robustness and preservation across semantic change
...
instance, a specification determines a single outcome, and provenance explains
how that outcome was derived.
In many systems of interest, however, a single input does not determine a unique
meaning.
Concurrency, negation, and underspecification may admit multiple admissible
outcomes, depending on how semantic choices are resolved.

This section introduces a minimal semantic framework for reasoning about such
ambiguity.
We model specifications that admit multiple admissible outcomes, and the
semantic commitments—called \emph{interpretations}—that refine ambiguity until
meaning becomes determinate.
The presentation is deliberately lightweight: we focus only on the structure
needed to define and explain interpretation provenance, deferring complexity,
algebraic structure, and optimization questions to later sections.
...
partial order representing known precedence constraints among events.
\end{definition}
...
No total order, timeline, or execution schedule is assumed.
Histories therefore represent semantic information, not executions.

We write $\Hist$ for the class of histories under consideration.
This choice is parametric: the nature of events and precedence constraints is
domain-specific and fixed by the specification.
...
Intuitively, histories grow by accumulating events and precedence constraints,
...
An \emph{observation} is an element of a set $\Obs$ of fully specified semantic objects.
\end{definition}
...
This notion of ambiguity—multiplicity in $\Spec(H) \subseteq \Obs$—is distinct from
uncertainty represented \emph{within} the observation domain $\Obs$ itself (e.g.,
probabilistic or multi-valued observations).
Interpretation provenance addresses ambiguity at the level of admissible
observations.
...
\[
  \varphi : \Hist \times 2^{\Obs} \to 2^{\Obs}
\]
such that for all histories $H$ and observation sets $S \subseteq \Obs$:
(\emph{Shrinkage}) $\varphi(H,S) \subseteq S$, and
(\emph{Feasibility}) if $S \neq \emptyset$ then $\varphi(H,S) \neq \emptyset$.
\end{definition}

Interpretive predicates correspond to the refinement operators studied in our
earlier work~\cite{hellerstein2026interpretation}.
They restrict admissible observations without introducing new ones or rendering
the specification infeasible.

Crucially, an interpretive predicate may depend on the \emph{entire current set}
of admissible observations.
That is, $\varphi(H,S)$ need not be definable as a pointwise test on individual
observations, but may instead enforce semantic commitments whose effect depends
on which alternatives remain admissible.
...
Sequential refinement therefore corresponds to conditioning on multiple semantic
commitments, whose interaction need not commute.
...
can matter.
This non-commutativity induces semantic dependency, studied in later sections.
...
Resolution is defined pointwise in the history.
A resolved specification need not produce a single global observation; rather,
for each history $H$, it determines a unique admissible observation.
Resolution therefore corresponds to determinism of $\Spec : \Hist \to \Obs$.
...
\[
  \Obs_H^I \triangleq (\Spec \mid I)(H).
\]
We say that two outcomes $o,o' \in \Obs_H^I$ are \emph{equivalent under $I$ at $H$},
...
\[
  \Obs_H^I \big/ {\sim_{I,H}},
\]
obtained by identifying admissible outcomes that are equivalent under $I$.
\end{definition}

The equivalence relation $\sim_{I,H}$ is total on the restricted domain $\Obs_H^I$.
...
rather than a single global quotient on $\Obs$.
\end{remark}
...
As a result, some observations become invariant across all resolving
interpretations precisely because the available predicates never discriminate
between the outcomes on which those observations would differ.
This phenomenon motivates the robustness questions studied in
Section~\ref{sec:robustness}: robustness diagnoses observations that persist
...
Appendix~\ref{sec:appendix-datalog} summarizes, for each, the interpretive predicates
...
Here, \emph{observations} refer to observable query answers or derived facts (elements of $\Obs$), which correspond to (possibly partial) properties of admissible semantic objects as defined in Section~\ref{sec:prelim}.
